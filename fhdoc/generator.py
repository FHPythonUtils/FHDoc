...
from fhdoc.loader import Loader, LoaderError
from fhdoc.processors.smart import SmartDocstringProcessor
from fhdoc.ast_parser.module_record_list import ModuleRecordList
from fhdoc.md_document import MDDocument
from fhdoc.utils import make_title
...
from fhdoc.settings import FIND_IN_SOURCE_LABEL
...
class Generator:
	...
	def __init__(self, input_path, # type: Path
	...
	toc_depth=1, # type: int
		...
		self._logger.debug("Generating source map for {} source files"
		.format(len(self._source_paths)))
		...
		self._docstring_links_re = re.compile(r"`+(?:{})\.\S+`+".format(
		package_names_re_expr))
		self._prepare_index()
	...
	def cleanup_old_docs(self):
		...
		for doc_path in PathFinder(self._output_path).glob("**/*.md"):
			...
			self._logger.info("Deleting orphaned doc file {}"
			.format(self._root_path_finder.relative(doc_path)))
			doc_path.unlink()
		...
		for orphaned_dir in orphaned_dirs:
			self._logger.info("Deleting orphaned directory {}"
			.format(self._root_path_finder.relative(orphaned_dir)))
			orphaned_dir.rmdir()
	...
	def _generate_doc(self, module_record, md_document):
		...
		self._logger.debug("Generating doc {} for {}"
		.format(self._root_path_finder.relative(md_document.path),
		...
		if self._source_code_url:
			relative_path_str = self._root_path_finder.relative(module_record
		...
		self._render_docstring(module_record=module_record, record=module_record,
		...
		module_record.import_string, max_depth=self._toc_depth,
		md_document=md_document, start_level=2,
		)
	...
	def _build_breadcrumbs_string(self, module_record, md_document):
		...
		for parent_import_string in parent_import_strings:
			...
			import_string_breadcrumbs.append(
			md_document
			.render_doc_link(parent_module_record.title, target_path=output_path,
	...
	def generate_docs(self):
		...
		"""
		self._logger.debug("Generating docs for {} to {}"
	...
	def generate_index(self):
		...
		.format(self._root_path_finder.relative(self.md_index.path)))
	...
	def generate_modules(self):
		...
		"""
		self._logger.debug("Generating {}"
		.format(self._root_path_finder.relative(self.md_modules.path)))
		with self.md_modules as md_modules:
			...
			modules_toc_lines = self._build_modules_toc_lines(
			import_string=ImportString(""), max_depth=10, md_document=md_modules,
			start_level=1,
			)
	...
	def _generate_module_doc_lines(self, module_record, md_document):
		...
		for record in module_record.iter_records():
			...
			source_link = md_document.render_doc_link(
			title=FIND_IN_SOURCE_LABEL, target_path=source_path,
			anchor="L{}".format(source_line_number),
			)
			if self._source_code_url:
				relative_path_str = self._root_path_finder.relative(source_path).as_posix()
				source_link = md_document.render_link(
				title=FIND_IN_SOURCE_LABEL, link="{}{}#L{}".format(self._source_code_url,
				relative_path_str, source_line_number),
				)
			...
			self._render_docstring(module_record=module_record, record=record,
			md_document=md_document)
		...
		for match in self._short_link_re.findall(docstring):
			...
			if not related_record:
				...
				if related_module_record:
					...
					target_path = self._loader.get_output_path(related_module_record
					.source_path)
			...
			if isinstance(related_record, AttributeRecord):
				parent_related_record = module_record.find_record(related_record
				.import_string.parent)
			...
			self._logger.debug("Adding local link '{}' to '{}'"
	...
	def _render_docstring(self, module_record, record, md_document):
		...
		for import_string in related_import_strings:
			...
			link = md_document.render_doc_link(title, target_path=target_path,
		...
		for link in links:
			...
			self._logger.debug("Adding link `{}` to `{}` `See also` section"
			.format(title, record.title))
